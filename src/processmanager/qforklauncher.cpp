...
/*!
  \headerfile <qforklauncher.h>
  \title QForklauncher
  \ingroup funclists

  \brief The <qforklauncher.h> file provides the \l{qForkLauncher()} function
  to convert a runtime into a forking runtime factory.
 */

/*!
  \fn void qForkLauncher(int *argc, char ***argv)
  \relates <qforklauncher.h>
  ...
  The qForkLauncher function makes it easy to convert any program into
  a self-forking parent process.  Simply call \c{qForkLauncher()} just
  ...
  The \c{qForkLauncher} function grabs control of the process and listens
  on STDIN for JSON-formatted messages that follow the remote protocol.
  This matches logically with the \l{QPipeProcessBackendFactory} class, which
  ...
  Any processes creation requests to the \l{QPipeProcessBackendFactory} will
  be sent to \c{myForkProgram}.  The \c{qForkLauncher()} function will
  receive the process creation request, call \c{fork()} to create a child
  process, and then return from the \c{qForkLauncher()} function call
  ...
  \l{QPipeProcessBackendFactory}.

  The process creation routine passes a complete \l{QProcessInfo} record,
  ...
  command line arguments \e{after} you return from \c{qForkLauncher()}
...
{
    QJsonObject message;
    message.insert(QRemoteProtocol::event(), QRemoteProtocol::output());
    message.insert(QRemoteProtocol::id(), id);
...
static void fixProcessState(const QProcessInfo& info, int *argc_ptr, char ***argv_ptr)
{
    // Fix the UID & GID values
    ::setpgid(0,0);
    if (info.contains(QProcessInfoConstants::Gid))
        ::setgid(info.gid());
    if (info.contains(QProcessInfoConstants::Uid))
        ::setuid(info.uid());
    ...
    // Fix the environment
    if (info.contains(QProcessInfoConstants::Environment)) {
        const char *entry;
    ...
    if (info.contains(QProcessInfoConstants::Arguments)) {
        const int argc = info.arguments().size() + 1;
        char **argv = new char *[argc + 1];
        if (info.contains(QProcessInfoConstants::Program))
    ...
    else if (info.contains(QProcessInfoConstants::Program)) {
...
void ChildProcess::processFdSet(QByteArray& outgoing, fd_set& rfds, fd_set& wfds)
{
    ...
    if (FD_ISSET(m_stdout, &rfds)) {  // Data to read
        ...
        if (m_outbuf.size())
            copyToOutgoing(outgoing, QRemoteProtocol::standardout(), m_outbuf, m_id);
    }
    if (FD_ISSET(m_stderr, &rfds)) {  // Data to read
        readToBuffer(m_stderr, m_errbuf);
        if (m_errbuf.size())
            copyToOutgoing(outgoing, QRemoteProtocol::standarderror(), m_errbuf, m_id);
    }
    if (m_state == SentSigTerm && m_timer.hasExpired(m_timeout)) {
        m_state = SentSigKill;
        QProcUtils::sendSignalToProcess(m_pid, SIGKILL);
    }
}
...
void ChildProcess::stop(int timeout)
{
    if (m_state == Running) {
        ...
        if (timeout > 0) {
            ...
            m_timeout = timeout;
            m_timer.start();
            QProcUtils::sendSignalToProcess(m_pid, SIGTERM);
        }
        else {
            m_state = SentSigKill;
            QProcUtils::sendSignalToProcess(m_pid, SIGKILL);
...
{
    if (!QProcUtils::setOomAdjustment(m_pid, oomAdjustment))
...
void ChildProcess::sendStateChanged(QByteArray& outgoing, QProcess::ProcessState state)
{
    ...
    msg.insert(QRemoteProtocol::id(), m_id);
...
void ChildProcess::sendStarted(QByteArray& outgoing)
{
    ...
    msg.insert(QRemoteProtocol::id(), m_id);
    msg.insert(QRemoteProtocol::pid(), m_pid);
    outgoing.append(QJsonDocument(msg).toBinaryData());
}
...
void ChildProcess::sendError(QByteArray& outgoing, QProcess::ProcessError err, const QString& errString)
{
    ...
    msg.insert(QRemoteProtocol::errorString(), errString);
    outgoing.append(QJsonDocument(msg).toBinaryData());
}
...
{
    if (message.value(QRemoteProtocol::remote()).toString() == QRemoteProtocol::halt()) {
        ...
        } else if (command == QRemoteProtocol::start()) {
        ...
        } else if (command == QRemoteProtocol::write()) {
            ChildProcess *child = m_children.value(id);
            if (child)
                child->write(QByteArray::fromBase64(message.value(QRemoteProtocol::data()).toString().toLatin1()));
        }
    }
    return false;
}
...
void qForkLauncher(int *argc, char ***argv )
{
