...
class Q_ADDON_PROCESSMANAGER_EXPORT QTimeoutIdleDelegate : public QIdleDelegate
{
    Q_OBJECT
    Q_PROPERTY(int idleInterval READ idleInterval WRITE setIdleInterval NOTIFY idleIntervalChanged)

public:
    explicit QTimeoutIdleDelegate(QObject *parent = 0);
...
private:
    Q_DISABLE_COPY(QTimeoutIdleDelegate)
    QTimer m_timer;
};
